...
//#include "external/mini_al.c"   // Implemented in mini_al.c. Cannot implement this here because it conflicts with Win32 APIs such as CloseWindow(), etc.
...
typedef struct RayAudioBuffer RayAudioBuffer;
struct RayAudioBuffer {
...
static RayAudioBuffer *firstAudioBuffer = NULL;   
static RayAudioBuffer *lastAudioBuffer = NULL;
...
RayAudioBuffer *CreateAudioBuffer(mal_format format, mal_uint32 channels, mal_uint32 sampleRate, mal_uint32 bufferSizeInFrames, AudioBufferUsage usage);
void DeleteAudioBuffer(RayAudioBuffer *audioBuffer);
bool IsAudioBufferPlaying(RayAudioBuffer *audioBuffer);
void PlayAudioBuffer(RayAudioBuffer *audioBuffer);
void StopAudioBuffer(RayAudioBuffer *audioBuffer);
void PauseAudioBuffer(RayAudioBuffer *audioBuffer);
void ResumeAudioBuffer(RayAudioBuffer *audioBuffer);
void SetAudioBufferVolume(RayAudioBuffer *audioBuffer, float volume);
void SetAudioBufferPitch(RayAudioBuffer *audioBuffer, float pitch);
void TrackAudioBuffer(RayAudioBuffer *audioBuffer);
void UntrackAudioBuffer(RayAudioBuffer *audioBuffer);
...
static mal_uint32 OnSendAudioDataToDevice(mal_device *pDevice, mal_uint32 frameCount, void *pFramesOut)
{
    ...
    {
        for (RayAudioBuffer *audioBuffer = firstAudioBuffer; audioBuffer != NULL; audioBuffer = audioBuffer->next)
        {
...
static mal_uint32 OnAudioBufferDSPRead(mal_dsp *pDSP, mal_uint32 frameCount, void *pFramesOut, void *pUserData)
{
    RayAudioBuffer *audioBuffer = (RayAudioBuffer *)pUserData;
...
RayAudioBuffer *CreateAudioBuffer(mal_format format, mal_uint32 channels, mal_uint32 sampleRate, mal_uint32 bufferSizeInFrames, AudioBufferUsage usage)
{
    RayAudioBuffer *audioBuffer = (RayAudioBuffer *)calloc(sizeof(*audioBuffer) + (bufferSizeInFrames*channels*mal_get_bytes_per_sample(format)), 1);
    if (audioBuffer == NULL)
    {
...
Sound LoadSoundFromWave(Wave wave)
{
    ...
    if (wave.data != NULL)
    {
        ...
        RayAudioBuffer* audioBuffer = CreateAudioBuffer(DEVICE_FORMAT, DEVICE_CHANNELS, DEVICE_SAMPLE_RATE, frameCount, AUDIO_BUFFER_USAGE_STATIC);
...
{
#if USE_MINI_AL
    DeleteAudioBuffer((RayAudioBuffer *)sound.audioBuffer);
#else
...
{
#if USE_MINI_AL
    RayAudioBuffer *audioBuffer = (RayAudioBuffer *)sound.audioBuffer;
    if (audioBuffer == NULL)
    {
...
{
#if USE_MINI_AL
    PlayAudioBuffer((RayAudioBuffer *)sound.audioBuffer);
#else
...
{
#if USE_MINI_AL
    PauseAudioBuffer((RayAudioBuffer *)sound.audioBuffer);
#else
...
{
#if USE_MINI_AL
    ResumeAudioBuffer((RayAudioBuffer *)sound.audioBuffer);
#else
    ALenum state;
...
{
#if USE_MINI_AL
    StopAudioBuffer((RayAudioBuffer *)sound.audioBuffer);
#else
    alSourceStop(sound.source);
#endif
}
...
{
#if USE_MINI_AL
    return IsAudioBufferPlaying((RayAudioBuffer *)sound.audioBuffer);
#else
    bool playing = false;
    ALint state;
...
{
#if USE_MINI_AL
    SetAudioBufferVolume((RayAudioBuffer *)sound.audioBuffer, volume);
#else
...
{
#if USE_MINI_AL
    SetAudioBufferPitch((RayAudioBuffer *)sound.audioBuffer, pitch);
#else
...
{
#if USE_MINI_AL
    RayAudioBuffer *audioBuffer = (RayAudioBuffer *)music->stream.audioBuffer;
    if (audioBuffer == NULL)
    {
...
void UpdateMusicStream(Music music)
{
    ...
    unsigned int subBufferSizeInFrames = ((RayAudioBuffer *)music->stream.audioBuffer)->bufferSizeInFrames/2;
...
AudioStream InitAudioStream(unsigned int sampleRate, unsigned int sampleSize, unsigned int channels)
{
    ...
    RayAudioBuffer *audioBuffer = CreateAudioBuffer(formatIn, stream.channels, stream.sampleRate, subBufferSize*2, AUDIO_BUFFER_USAGE_STREAM);
    if (audioBuffer == NULL)
    {
...
{
#if USE_MINI_AL
    DeleteAudioBuffer((RayAudioBuffer *)stream.audioBuffer);
#else
...
{
#if USE_MINI_AL
    RayAudioBuffer *audioBuffer = (RayAudioBuffer *)stream.audioBuffer;
    if (audioBuffer == NULL)
    {
...
{
#if USE_MINI_AL
    PlayAudioBuffer((RayAudioBuffer *)stream.audioBuffer);
#else
...
{
#if USE_MINI_AL
    PauseAudioBuffer((RayAudioBuffer *)stream.audioBuffer);
#else
...
{
#if USE_MINI_AL
    ResumeAudioBuffer((RayAudioBuffer *)stream.audioBuffer);
#else
    ALenum state;
...
{
#if USE_MINI_AL
    return IsAudioBufferPlaying((RayAudioBuffer *)stream.audioBuffer);
#else
    bool playing = false;
    ALint state;
...
{
#if USE_MINI_AL
    StopAudioBuffer((RayAudioBuffer *)stream.audioBuffer);
#else
    alSourceStop(stream.source);
#endif
}
...
{
#if USE_MINI_AL
    SetAudioBufferVolume((RayAudioBuffer *)stream.audioBuffer, volume);
#else
...
{
#if USE_MINI_AL
    SetAudioBufferPitch((RayAudioBuffer *)stream.audioBuffer, pitch);
#else
