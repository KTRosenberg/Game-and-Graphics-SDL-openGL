...
struct Render_Batch {
    ...
    Vertex_Attribute_Array vao_triangles;
    Vertex_Buffer_Data triangle_buffer;
    Vertex_Attribute_Array vao_lines;
    Vertex_Buffer_Data line_buffer;
    Shader shader;

    Uniform_Location MAT_LOC;
    ...
    bool init(Mat4 projection_matrix)
    {
        ...
        Vertex_Attribute_Array_init(&vao_triangles, attribute_stride);
        glBindVertexArray(vao_triangles);
        ...
        Vertex_Attribute_Array_init(&vao_lines, attribute_stride);
        glBindVertexArray(vao_lines);
            Vertex_Buffer_Data_init_inplace(
                &line_buffer, 
                SD_RENDER_BATCH_SIZE * attribute_stride,
    ...
    void free(void)
    {
        Vertex_Attribute_Array_delete(&vao_triangles);
        Vertex_Attribute_Array_delete(&vao_lines);
        Vertex_Buffer_Data_delete_inplace(&triangle_buffer);
        Vertex_Buffer_Data_delete_inplace(&line_buffer);
...
template<usize SD_RENDER_BATCH_SIZE> inline bool layer_init(sd::Render_Batch<SD_RENDER_BATCH_SIZE>* ctx, Mat4 projection_matrix)
{
    ...
    Vertex_Attribute_Array_init(&ctx->vao_triangles, attribute_stride);
    glBindVertexArray(ctx->vao_triangles);
    ...
    Vertex_Attribute_Array_init(&ctx->vao_lines, attribute_stride);
    glBindVertexArray(ctx->vao_lines);
        Vertex_Buffer_Data_init_inplace(
            &ctx->line_buffer, 
            SD_RENDER_BATCH_SIZE * attribute_stride,
...
{
    Vertex_Attribute_Array_delete(&ctx->vao_triangles);
    Vertex_Attribute_Array_delete(&ctx->vao_lines);
    Vertex_Buffer_Data_delete_inplace(&ctx->triangle_buffer);
    Vertex_Buffer_Data_delete_inplace(&ctx->line_buffer);
