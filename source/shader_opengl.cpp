                        ...
                        const std::string& frag_addons) 
{
    ...
    bool status = false;
    std::string vs = Shader_retrieve_src_txt_from_file(vertex_path.c_str(), &status);
    if (!status) {
        fprintf(stderr, "%s %s\n", "ERROR: CANNOT FIND VERTEX SHADER FILE", vertex_path.c_str());
        return false;
    }
    std::string fs = Shader_retrieve_src_txt_from_file(fragment_path.c_str(), &status);
    if (!status) {
        fprintf(stderr, "%s %s\n", "ERROR: CANNOT FIND FRAGMENT SHADER FILE", fragment_path.c_str());
        return false;
    } 
    
    return Shader_load_from_src_txt(
        shader,
        vs,
        fs,
        vert_addons,
...
{
    for (usize i = 0; i < catalogue.shaders.count; i += 1) {
        Shader_deinit_program(catalogue.shaders[i]);
        GL_GET_ERRORS();
    }
    deallocate(&catalogue.shaders);
    clear(&catalogue.shaders);
}
