...
//     );
    ...
    Texture texture[2];
    texture[0].load(IMG_PATH_1, true);
    texture[1].load(IMG_PATH_2, true);
    ...
    while (keep_running) {
        ...
        for (GLuint i = 0; i < 10; i++) { 
            scene.m_model = ident_mat;
            scene.m_model = glm::translate(scene.m_model, plane_translations[i]);         
            scene.m_model = glm::rotate(scene.m_model, elapsed + glm::radians(20.0f * i), glm::vec3(1.0, 0.3, 0.5));
